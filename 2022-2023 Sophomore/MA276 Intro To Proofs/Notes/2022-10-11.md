...
<u>proof</u>. $(\to)$ Suppose $f^{-1}$ is a function. We want to show that $f$ is injective and surjective. Suppose $x,y \in A$ such that $f(x) = f(y)$. So $(x,f(x)),(y,f(y))\in f$. So $(f(x),x),(f(y),y)\in f^{-1}$. Since $f^{-1}$ is a function, we know that $x=y$. This $f$ is injective.

(Surjective)  Let $b \in B$. We want to show $\exists a \in A$ such that $(a,b)\in f.$ Since $f^{-1}$ is a function, $\exists a \in A$ such that $(b,a) \in f^{-1}$. So $(a,b)\in F$